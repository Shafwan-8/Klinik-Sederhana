...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom text-dark">
  <h1 class="h2">Data Jumlah Transaksi Pasien</h1>
</div>
...
@endif --}}
<div class="p-0 d-flex justify-content-end px-3 py-2">
    <form action="{{ route('download-pdf-transaction') }}" method="post" target="__blank">
        @csrf
    ...
    <form action="{{ route('view-pdf-transaction') }}" method="post" target="__blank" class="ml-2">
        @csrf
            <input type="hidden" name="start_date" value="{{ request('start_date') }}">
            <input type="hidden" name="end_date" value="{{ request('end_date') }}">
